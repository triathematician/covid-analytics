...
fun printAllForecasts(region: String) {
    CovidForecasts.allForecasts.filter { it.region == region }.forEach { it.log() }
}
...
fun `compute moving average and doubling time series`(metric: String, min: Double) {
    CovidTimeSeriesSources.dailyReports()
            ...
            .sortedByDescending { it.values.deltas().movingAverage(3).last() }
            .forEach {
                it.log()
                it.values.deltas().movingAverage(5).log(prefix = "  ")
                it.values.movingAverage(5).doublingTimes().log(prefix = "  ")
                it.values.deltas().movingAverage(5).map { risk_PerCapitaDeathsPerDay(it).level }.log(prefix = "  ")
...
fun `compute long term trends for italy and south korea`() {
    ...
    italy.values.deltas().movingAverage(7).log(prefix = "ITALY deltas\t")
    ...
    `south korea`.values.deltas().movingAverage(7).log(prefix = "SOUTH KOREA deltas\t")
