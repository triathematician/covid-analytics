...
import tri.util.logger
import tri.util.measureTime

// this class contains utilities for processing timeseries data for [Usa] area types

/** The log level for logging geographic region aggregations. */
var LOG_LEVEL = java.util.logging.Level.INFO!!
...
fun List<TimeSeries>.withAggregate(cbsa: Boolean = false, state: Boolean = false, regional: Boolean = false, censusRegional: Boolean = false, national: Boolean = false): List<TimeSeries> {
    ...
    if (cbsa) measureTime { res += aggregateByCbsa().flatMap { it.value } }
        .also { logger<TimeSeries>().log(LOG_LEVEL, "  aggregated $size records to CBSA in $it") }
    if (state) measureTime { res += aggregateByState().flatMap { it.value } }
        .also { logger<TimeSeries>().log(LOG_LEVEL, "  aggregated $size records to State in $it") }
    if (regional) measureTime { res += aggregateByRegion().flatMap { it.value } }
        .also { logger<TimeSeries>().log(LOG_LEVEL, "  aggregated $size records to Region in $it") }
    ...
    }.also { logger<TimeSeries>().log(LOG_LEVEL, "  aggregated $size records to Census Regional in $it") }
    if (national) measureTime { res += aggregateToNational() }
        .also { logger<TimeSeries>().log(LOG_LEVEL, "  aggregated $size records to National in $it") }
    return res.flatten()
}
...
fun List<TimeSeries>.aggregateByRegionXY(): Map<String, List<TimeSeries>> {
    return filter { xyRegionOf(UsaAreaLookup.area(it.areaId)) != null }
        .groupBy { listOf(it.source, xyRegionOf(UsaAreaLookup.area(it.areaId)), it.metric, it.qualifier) }.mapValues { data ->
        (data.key[1] as? UsRegionInfo)?.let { data.value.sum(it.id) }
    }.mapNotNull { it.value }.groupBy { (UsaAreaLookup.area(it.areaId) as UsRegionInfo).id }
}
...
fun List<TimeSeries>.aggregateByCensusRegion(): Map<String, List<TimeSeries>> {
    return filter { censusRegionOf(UsaAreaLookup.area(it.areaId)) != null }
        .groupBy { listOf(it.source, censusRegionOf(UsaAreaLookup.area(it.areaId)), it.metric, it.qualifier) }.mapValues { data ->
        (data.key[1] as? UsRegionInfo)?.let { data.value.sum(it.id) }
    }.mapNotNull { it.value }.groupBy { (UsaAreaLookup.area(it.areaId) as UsRegionInfo).id }
}
...
fun List<TimeSeries>.aggregateByCensusDivision(): Map<String, List<TimeSeries>> {
    return filter { censusDivisionOf(UsaAreaLookup.area(it.areaId)) != null }
        .groupBy { listOf(it.source, censusDivisionOf(UsaAreaLookup.area(it.areaId)), it.metric, it.qualifier) }.mapValues { data ->
        (data.key[1] as? UsRegionInfo)?.let { data.value.sum(it.id) }
    }.mapNotNull { it.value }.groupBy { (UsaAreaLookup.area(it.areaId) as UsRegionInfo).id }
}
