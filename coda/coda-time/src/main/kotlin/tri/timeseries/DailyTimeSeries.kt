...
data class DailyTimeSeries(
    ...
    fun copyWithDataSince(firstDate: LocalDate) = dropFirst(ChronoUnit.DAYS.between(start, maxOf(start, firstDate)).toInt())
     ...
     */
    fun copyExtendedThrough(date: LocalDate, fill: TimeSeriesFillStrategy): DailyTimeSeries {
        if (!date.isAfter(end)) {
            return this
        }
        val daysToAdd = ChronoUnit.DAYS.between(end, date)
    ...
    /** Copy after dropping first n values. */
    fun dropLast(n: Int): DailyTimeSeries {
    ...
    operator fun plus(n: DailyTimeSeries) = mergeSeries(this, n) { a, b -> a + b }
    operator fun minus(n: DailyTimeSeries) = mergeSeries(this, n) { a, b -> a - b }
    operator fun times(n: DailyTimeSeries) = mergeSeries(this, n) { a, b -> a * b }
    operator fun div(n: DailyTimeSeries) = mergeSeries(this, n) { a, b -> a / b }
     ...
     */
    fun minusMustBePositive(n: DailyTimeSeries) = minus(n).let {
     ...
     */
    fun coerceIncreasing(): DailyTimeSeries {
     ...
     */
    fun replaceZerosWithPrevious(): DailyTimeSeries {
        val res = values.toMutableList()
        for (i in 1 until res.size) {
    ...
    fun restrictToFiniteValues(): DailyTimeSeries {
        val firstFinite = values.indexOfFirst { it.isFinite() }
        return when {
            firstFinite > 0 -> dropFirst(firstFinite)
            else -> this
        }
    }

    /** If time series starts with more than max zeros, trim so that it has max zeros. */
    fun restrictNumberOfStartingZerosTo(max: Int): DailyTimeSeries {
